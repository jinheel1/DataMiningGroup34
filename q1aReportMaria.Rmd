...
The screeplot of PC object shows a decreasing variance explained, as expected. To determine the amount of PC's I want to determine it is helpful to see the increasing gains in variance explained be each new PC. Using this it seems that the elbow of cumulative variance explained is passed around 60. So going down to 60 PC's seems to be the reasonable next steps.
```{r}
...
There's also the variables clustered in the center lower right half, these are pointing away from the two extreme clusters which means they are unlike those two groups, they have low positive loadings on dimension 1 and dimension 2.
```{r}

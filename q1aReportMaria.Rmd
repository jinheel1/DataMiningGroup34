...
```{r}
pam.res = pam(X, k = 2)
pam.res.64.2 = pam(pc.65, k = 2)
pam.res.64.4 = pam(pc.65, k = 4)

pam.res$clustering
pam.res.64.2$clustering
pam.res.64.4$clustering
# Error proportion: the number of incorrect classification divided the number of rows
kmed_error = best_cluster(pam.res$clustering, y$label)[3]
cat("Error:", kmed_error * 100, "%")

kmed_error.64.2 = best_cluster(pam.res.64.2$clustering, y$label)[3]
cat("Error:", kmed_error.64.2 * 100, "%")

kmed_error.64.4 = best_cluster(pam.res.64.4$clustering, y$label)[3]
